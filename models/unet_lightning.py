...
class UNet_Lightning(pl.LightningModule):
    def __init__(self, UNet_params: dict, params: dict,
        ...
        self.model = Base_UNET3D(in_channels=self.in_channels,
                                 start_filts=self.start_filts, dropout_rate=self.dropout_rate)
    ...
    def validation_step(self, batch, batch_idx, phase='val'):
        ...
        values = {'val_acc': acc, 'val_recall': recall,
                  'val_precision': precision, 'val_F1': F1, 'val_iou': iou, 'val_CSI': csi}
    ...
    def test_step(self, batch, batch_idx, phase='test'):
        ...
        # LOGGING
        self.log(f'{phase}_loss', loss, batch_size=self.bs, sync_dist=True)
        values = {'test_acc': acc, 'test_recall': recall, 'test_precision': precision,
                  'test_F1': F1, 'test_iou': iou, 'test_CSI': csi}
        self.log_dict(values, batch_size=self.bs, sync_dist=True)
    ...
    def configure_optimizers(self):
        if VERBOSE:
            print("Learning rate:",
                  self.params["lr"], "| Weight decay:", self.params["weight_decay"])
