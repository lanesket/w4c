...
def load_sample_ids(data_split, splits_df, len_seq_in, len_seq_predict, regions, generate_pkl, path=''):
    ...
    if generate_pkl:
        idxs_r = generate_and_cache_sequences(
            data_split, splits_df, len_seq_in, len_seq_predict, regions, path)
        ...
        idxs_r = generate_sample_ids(
            data_split, splits_df, len_seq_in, len_seq_predict, regions)
    return idxs_r
...
def generate_sample_ids(data_split, splits_df, len_seq_in, len_seq_predict, regions):
    ...
    if data_split == "training":
        idxs_r = get_training_idxs(
            splits_df, len_seq_in, len_seq_predict, data_split, regions[0])
        for r in range(1, len(regions)):  # append rest of regions to df
            idxs = get_training_idxs(
                splits_df, len_seq_in, len_seq_predict, data_split, regions[r])
            idxs_r = idxs_r + idxs
        ...
        idxs_r = get_test_heldout_idxs(
            splits_df, len_seq_in, data_split, regions[0])
        for r in range(1, len(regions)):  # append rest of regions to df
            idxs = get_test_heldout_idxs(
                splits_df, len_seq_in, data_split, regions[r])
            idxs_r = idxs_r + idxs
    return idxs_r
...
def get_io_validation_times(df, start_index, len_seq_in, len_seq_predict, region):
    ...
    if end_index < len(df):
        ...
        # convert input/output sequence to lists and return
            ...
            in_seq = [start_index + i for i in range(len_seq_in)]
            out_seq = [start_index + len_seq_in +
                       i for i in range(len_seq_predict)]
...
def get_sequence(seq, root, data_split, region, product, bands, preprocess=None, swap_time_ch=False, ds=None):
    ...
    mask_seq = []
    for s in seq:
        prods, masks = get_file(s, root, data_split,
                                region, product, bands, preprocess, ds)
...
def get_cuda_memory_usage(gpus):
    ...
    for gpu in gpus:
        ...
        print("GPU", gpu, "CUDA memory reserved:",
              r, "allocated:", a, "free:", f)
...
def prepare_crop(location, opera_k=1):
    ...
    context_shape = {
        'x_start': position[0], 'y_start': position[1], 'size': context_size}
...
def tensor_to_submission_file(predictions, predict_params):
    ...
    submission_path = os.path.join('submission', str(
        predict_params['year_to_predict']), submission_file_name)
